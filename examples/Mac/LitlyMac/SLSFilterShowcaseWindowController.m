...
- (void)setupFilter {
    ...
    if (!filter){
        filter = [[GPUImageLitlyFilter alloc] init];
        [filter addTarget:(GPUImageView*) self.glView];
    }
    
    int filterType = 0;
    
    switch (filterType) {
        case GPUIMAGE_LITELY:
        {
            [inputCamera addTarget:filter];
            break;
        }
        case GPUIMAGE_SOBEL_EDGE_DETECTION_LITELY:
        {
            
            GPUImageSobelEdgeDetectionFilter* sobelEdgeDetectionFilter = [[GPUImageSobelEdgeDetectionFilter alloc] init];
            
            [inputCamera addTarget:sobelEdgeDetectionFilter];
            
            [sobelEdgeDetectionFilter addTarget:filter];
            break;
        ...
        case GPUIMAGE_PINCH:
        {
            ...
            [pinchFilter setCenter:CGPointMake(0.5, 0.5)];
            [pinchFilter setRadius:10.];
            [pinchFilter setScale:0.5];
            [inputCamera addTarget: pinchFilter];
            [pinchFilter addTarget:filter];
            break;
...
- (void)changeSelectedRow:(NSUInteger)newRowIndex;
{
    ...
    switch(currentlySelectedRow)
    {
        ...
        {
            currentlySelectedFilter = [[GPUImageLitlyFilter alloc] init];
            self.minimumSliderValue = 1.0;
            self.maximumSliderValue = 20.0;
...
- (void)setCurrentSliderValue:(CGFloat)newValue;
{
    ...
    switch(currentlySelectedRow)
    {
        ...
        case GPUIMAGE_POSTERIZE: [(GPUImageLitlyFilter *)currentlySelectedFilter setSensitivity:_currentSliderValue]; break;
        case GPUIMAGE_TILTSHIFT:
        {
...
- (id)tableView:(NSTableView *)aTableView objectValueForTableColumn:(NSTableColumn *)aTableColumn row:(int)rowIndex
{
    ...
    switch(rowIndex)
    {
        ...
        case GPUIMAGE_POSTERIZE: tableRowTitle = @"Litly"; break;
